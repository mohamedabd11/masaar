...
function showApp(name, role, pages, user) {
  ...
  if (btn) showPage(btn);
  // حالة وهمية لاعتراض زر الرجوع في Android
  history.pushState({ masaar: true }, '', '');
}
...
// ── سجل التنقل الداخلي (لزر الرجوع في Android) ──
const _navHistory = [];

// تنقل للصفحة بدون تسجيل في السجل (يُستخدم من popstate)
function _goToPageId(pageId) {
  if (!AppState.currentPages.includes(pageId)) pageId = 'pg-dash';
  if (AppState.currentPageId !== pageId) resetPages();
  AppState.currentPageId = pageId;
  document.querySelectorAll('.sb-item').forEach(b =>
    b.classList.toggle('active', b.getAttribute('data-page') === pageId));
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  const page = g(pageId); if (page) page.classList.add('active');
...
function showPage(btn) {
  ...
  const prevPage = AppState.currentPageId;
  // حفظ الصفحة الحالية في السجل (للرجوع إليها لاحقاً)
  if (prevPage && prevPage !== pageId) {
    _navHistory.push(prevPage);
    // الحد الأقصى 20 صفحة في السجل
    if (_navHistory.length > 20) _navHistory.shift();
  }
  _goToPageId(pageId);
  closeSidebar();

  // تحديث ما يلزم عند الانتقال لصفحات معينة
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('popstate', () => {
    ...
    if (prev && AppState.currentPageId !== 'pg-dash') {
      // ارجع للصفحة السابقة
      _goToPageId(prev);
    } else if (AppState.currentPageId !== 'pg-dash') {
      // لا يوجد تاريخ — ارجع للرئيسية
      _goToPageId('pg-dash');
    } else {
      // نحن على الرئيسية — اسمح للمتصفح بالخروج
      return;
    }
    // أعد وضع الحالة الوهمية حتى تعمل المرة القادمة
    history.pushState({ masaar: true }, '', '');
  });
