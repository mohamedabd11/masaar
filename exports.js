...
export function importExcel(currentUser, writeAuditLog) {
  ...
  input.onchange = async (ev) => {
    ...
    try {
      ...
      // تحديد أعمدة الملف بمرونة
      const firstRow = rows[0];
      const keys     = Object.keys(firstRow);

      // دوال للبحث عن العمود بأسماء متعددة
      const findKey = (...names) => keys.find(k => names.some(n => k.includes(n))) || '';

      const kDate  = findKey('التاريخ','date','Date','تاريخ');
      const kRef   = findKey('السند','المرجع','ref','Ref','سند');
      const kPlate = findKey('اللوحة','plate','لوحة','شاحنة');
      const kType  = findKey('النوع','type','Type','نوع');
      const kDesc  = findKey('البيان','الوصف','desc','بيان','وصف');
      const kMet   = findKey('أمتار','متر','met','meters','م');
      const kRaw   = findKey('الأصلية','rawMeters','الكمية الأصلية','raw');
      const kDed   = findKey('الخصم','deducted','خصم');
      const kDeb   = findKey('وارد','deb','استلام','Deb');
      const kCrd   = findKey('صادر','crd','تسليم','Crd','مبلغ','amount');
      const kShift = findKey('وردية','الوردية','shift','Shift');
      const kNotes = findKey('ملاحظات','notes','Notes');

      // خريطة النوع من النص العربي
      const typeMap = {
      ...
      const preview = rows.slice(0,3).map((r,i) =>
        `${i+1}: ${r[kDate]||'—'} | ${r[kType]||'—'} | ${r[kDesc]||'—'}`
      ).join('
');

      const confirmMsg =
        `📊 تم قراءة "${file.name}" — شيت: "${sheetName}"
` +
      ...
      for (const r of rows) {
        ...
        if (date.includes('/')) {
          ...
          if (p.length === 3) {
            ...
            else date = `${p[0]}-${p[1].padStart(2,'0')}-${p[2].padStart(2,'0')}`;
          }
        }

        // تحديد النوع
        const rawType = String(r[kType]||'').trim();
        const type    = typeMap[rawType] || typeMap[Object.keys(typeMap).find(k=>rawType.includes(k.replace(/[📏📥📤💸]/g,'').trim()))||''] || 'expense';
        ...
        await addDoc(collection(db, COLL.ENTRIES), entry);
        count++;
      }
