...
import { initExports, updateReportFilter, setExpDate, exportExcel, exportPDF,
         ...
         exportUserReportExcel, exportJSON, importJSON, importExcel,
...
function subscribeToEntries() {
  ...
  AppState.unsubEntries = onSnapshot(q, snapshot => {
    ...
    invalidateCache();
    renderAll();
  }, err => { console.error('[Firestore] entries error:', err.code, err.message); toast(`⚠ خطأ (${err.code || err.message})`, 'err'); });
}
...
// ══════════════════════════════════════════════
function _initPWA() {
  // manifest.json و icon-192.png مربوطان مباشرة في index.html
  // لا يلزم أي إعداد ديناميكي
}
