...
function showPage(btn) {
  ...
  // دفع الصفحة لتاريخ المتصفح (لزر الرجوع في Android)
  if (pageId !== 'pg-dash') {
    history.pushState({ pageId }, '', '');
  }
  // إعادة تعيين الـ pagination عند الانتقال لصفحة جديدة
  if (prevPage !== pageId) resetPages();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // زر الرجوع في Android — يرجع للصفحة السابقة بدل الخروج
  window.addEventListener('popstate', (e) => {
    const targetPage = e.state?.pageId || 'pg-dash';
    const btn = document.querySelector(`.sb-item[data-page="${targetPage}"]`);
    if (btn) {
      // navigate without pushing to history again
      const prevId = AppState.currentPageId;
      AppState.currentPageId = targetPage;
      document.querySelectorAll('.sb-item').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
      const page = document.getElementById(targetPage);
      if (page) page.classList.add('active');
      setEl('pg-title', PAGE_TITLES[targetPage] || '');
    } else {
      // إذا لم توجد صفحة — اذهب للرئيسية
      navTo('pg-dash');
    }
  });
